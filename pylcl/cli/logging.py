...
from pylcl.cli.models import LogConfig
from pylcl.runtime import Frame
...
async def resolve_log_config(frame: Frame) -> LogConfig:
    ...
    """
    return LogConfig(
        log_dir=await frame.get("log_dir"),  # type: ignore[arg-type]
        log_file_name=await frame.get("log_file_name"),  # type: ignore[arg-type]
        log_level=await frame.get("log_level"),  # type: ignore[arg-type]
        log_format=await frame.get("log_format"),  # type: ignore[arg-type]
    )
...
async def create_logger(frame: Frame, identity: str) -> LoggerHandle:
    ...
    logger = logging.Logger(f"pylcl.cli.{identity}", numeric_log_level(config.log_level))
