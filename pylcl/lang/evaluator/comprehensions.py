...
async def _evaluate_comprehension(
    ...
    stream = _sequence(node.element, node.clauses, resolver, evaluate)
...
async def _sequence(
    ...
    """
    async for scope in _bindings(clauses, resolver, evaluate):
...
async def _dictionary(
    ...
    async for scope in _bindings(node.clauses, resolver, evaluate):
...
async def _bindings(
    ...
    async for item in iterate_values(iterable):
        ...
        if await _conditions(clause, scope, evaluate):
            async for nested in _bindings(clauses, scope, evaluate, index + 1):
                yield nested
