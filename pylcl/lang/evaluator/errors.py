...
from pylcl.ast import LclAssert, LclExceptHandler, LclRaise, LclTry
...
async def _evaluate_error_form(
    node: ErrorNode,
    resolver: Resolver,
    ...
    return await _try(node, resolver, evaluate)
...
async def _try(
    ...
    try:
        try:
            ...
            result = await _handlers(error, node.handlers, resolver, evaluate)
        return result
    finally:
...
async def _handlers(
    ...
    for handler in handlers:
        if handler.exception is None or await _matches(handler, error, resolver, evaluate):
            scope = resolver
...
def _wrap_failure(error: Exception, span: SourceSpan) -> LclEvaluationError:
