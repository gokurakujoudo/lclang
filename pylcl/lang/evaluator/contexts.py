...
async def _evaluate_with(
    ...
    """
    return await _enter_item(node, 0, resolver, evaluate)
...
async def _enter_item(
    ...
    value, exit_method = await _acquire(manager)
    scope = resolver
    if item.target is not None:
        scope = ScopedResolver({str(item.target): value}, resolver)
    try:
        result = await _enter_item(node, index + 1, scope, evaluate)
...
async def _acquire(manager: object) -> tuple[object, ExitMethod]:
    ...
    sync_manager = cast(_SyncProtocol, manager)
