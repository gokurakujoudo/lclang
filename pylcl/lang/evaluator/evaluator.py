...
from pylcl.lang.evaluator.functions import _create_function
...
def evaluate_sync(node: LclAstNode, resolver: ResolverSource = None) -> object:
    """Evaluate one AST from synchronous code using a private event loop.
    ...
    return asyncio.run(evaluate(node, resolver))
...
async def _evaluate_node(node: LclAstNode, resolver: Resolver) -> object:
    ...
    if isinstance(node, LclConstant):
        ...
        result = await _create_function(node, resolver, _evaluate)
