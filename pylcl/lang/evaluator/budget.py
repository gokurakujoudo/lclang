...
class _EvaluationGuard(Protocol):
...
_ACTIVE_GUARD: ContextVar[_EvaluationGuard | None] = ContextVar(
    "pylcl_evaluation_guard",
    default=None,
)
_EVALUATION_DEPTH: ContextVar[int] = ContextVar("pylcl_evaluation_depth", default=0)
...
@contextmanager
def _install_guard(guard: _EvaluationGuard) -> Iterator[None]:
...
def _enter_node(span: SourceSpan) -> Token[int] | None:
...
def _leave_node(token: Token[int] | None) -> None:
...
def _check_collection(size: int, span: SourceSpan) -> None:
