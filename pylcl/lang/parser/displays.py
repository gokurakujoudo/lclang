...
from pylcl.lang.parser.comprehensions import ComprehensionKind, parse_comprehension
from pylcl.lang.parser.stream import TokenStream
from pylcl.source import SourceSpan


class _DisplayParser:
    ...
    def _parenthesized(self) -> LclAstNode:
        ...
        if self.stream.current.kind is TokenKind.RPAREN:
            ...
            return LclTuple((), span=self._container_span(closing.span))
        first = self._sequence_element()
        if self.stream.current.kind is TokenKind.KW_FOR:
            return self._comprehension(first, ComprehensionKind.GENERATOR)
        if self.stream.match(TokenKind.COMMA) is None:
            closing = self.stream.expect(TokenKind.RPAREN, "expected closing parenthesis")
            return replace(first, span=self._container_span(closing.span))
        elements = [first]
        while self.stream.peek().kind is not TokenKind.RPAREN:
            elements.append(self._sequence_element())
            if self.stream.match(TokenKind.COMMA) is None:
                break
        closing = self.stream.expect(TokenKind.RPAREN, "expected closing tuple parenthesis")
        return LclTuple(tuple(elements), span=self._container_span(closing.span))

    def _list(self) -> LclAstNode:
        ...
        if self.stream.current.kind is TokenKind.RBRACKET:
            closing = self.stream.advance()
            return LclList((), span=self._container_span(closing.span))
        first = self._sequence_element()
        if self.stream.current.kind is TokenKind.KW_FOR:
            return self._comprehension(first, ComprehensionKind.LIST)
        elements = [first]
        ...
        return LclList(tuple(elements), span=self._container_span(closing.span))

    def _sequence_element(self) -> LclAstNode:
        ...
        return LclStarred(value, span=_merge_span(marker.span, value.span))

    def _braces(self) -> LclAstNode:
        ...
        if self.stream.current.kind is TokenKind.RBRACE:
            closing = self.stream.advance()
            return LclDict((), span=self._container_span(closing.span))
        if self.stream.current.kind is TokenKind.DOUBLE_STAR:
            marker = self.stream.advance()
            value = self.parse_nested()
            entry = LclDictUnpack(value, span=_merge_span(marker.span, value.span))
            if self.stream.peek().kind is TokenKind.KW_FOR:
                return self._comprehension(entry, ComprehensionKind.DICT)
            return self._dict([entry])
        first = self._sequence_element()
        if self.stream.match(TokenKind.COLON) is not None:
            value = self.parse_nested()
            entry = LclKeyValue(first, value, span=_merge_span(first.span, value.span))
            if self.stream.current.kind is TokenKind.KW_FOR:
                return self._comprehension(entry, ComprehensionKind.DICT)
            return self._dict([entry])
        if self.stream.current.kind is TokenKind.KW_FOR:
            return self._comprehension(first, ComprehensionKind.SET)
        return self._set([first])

    def _dict(self, entries: list[LclDictEntry]) -> LclDict:
        ...
        while True:
            ...
            if marker is not None:
                value = self.parse_nested()
                entries.append(LclDictUnpack(value, span=_merge_span(marker.span, value.span)))
                continue
            ...
            value = self.parse_nested()
            entries.append(LclKeyValue(key, value, span=_merge_span(key.span, value.span)))
        closing = self.stream.expect(TokenKind.RBRACE, "expected closing dictionary brace")
        return LclDict(tuple(entries), span=self._container_span(closing.span))

    def _set(self, elements: list[LclAstNode]) -> LclSet:
        ...
        while self.stream.match(TokenKind.COMMA) is not None:
            ...
            element = self._sequence_element()
        ...
        return LclSet(tuple(elements), span=self._container_span(closing.span))

    def _container_span(self, closing: SourceSpan) -> SourceSpan:
        ...
        """
        return _merge_span(self.opening.span, closing)
...
def parse_display(
    ...
    .. note::
       Comprehension clauses are intentionally deferred to M021.
    """
    return _DisplayParser(stream, parse_nested, parse_nonconditional).parse()
