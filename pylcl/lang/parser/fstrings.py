...
def _parse_fstring(token: Token) -> LclJoinedString:
    ...
    return _convert_value(value, token.span)


def _convert_value(value: FStringValue, span: SourceSpan) -> LclJoinedString:
    ...
    return LclJoinedString(
        values=tuple(_convert_part(part, span) for part in value.parts),
        span=span,
    )
...
def _convert_part(
    ...
    expression = _parse_field_expression(part.expression, span)
    format_spec = (
        None if part.format_spec is None else _convert_value(part.format_spec, span)
    )
...
def _parse_field_expression(source: str, span: SourceSpan) -> LclAstNode:
