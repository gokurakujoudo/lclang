...
class _PrimaryParser:
    ...
    def parse(self, value: LclAstNode) -> LclAstNode:
        ...
        while self.stream.current.kind in {
            ...
            if kind in {TokenKind.DOT, TokenKind.QUESTION_DOT}:
                value = self._attribute(value, safe=kind is TokenKind.QUESTION_DOT)
            elif kind is TokenKind.LBRACKET:
                value = self._subscript(value)
            else:
                value = self._call(value)
        return value

    def _attribute(self, value: LclAstNode, *, safe: bool) -> LclAstNode:
    ...
    def _subscript(self, value: LclAstNode) -> LclSubscript:
        ...
        if self.stream.match(TokenKind.COLON) is not None:
            index = self._slice(opening.span, None)
        else:
            first = self.parse_nested()
            if self.stream.match(TokenKind.COLON) is not None:
                index = self._slice(opening.span, first)
                ...
                index = LclTuple(
                    tuple(elements),
                    span=_merge_span(opening.span, closing.span),
                )
            else:
                closing = self.stream.expect(TokenKind.RBRACKET, "expected closing bracket")
                return LclSubscript(
                    value=value,
                    index=first,
                    span=_merge_span(value.span, closing.span),
                )
        return LclSubscript(
            value=value,
            index=index,
            span=_merge_span(value.span, index.span),
        )

    def _slice(self, opening: SourceSpan, lower: LclAstNode | None) -> LclSlice:
    ...
    def _call(self, function: LclAstNode) -> LclCall:
        ...
        return LclCall(
            ...
            span=_merge_span(function.span, closing.span),
        )
        ...
        *,
        ...
        if marker is not None:
            ...
            return LclKeywordUnpackArgument(
                value,
                span=_merge_span(marker.span, value.span),
            )
        ...
        if marker is not None:
            ...
            value = self.parse_nested()
            return LclStarArgument(value, span=_merge_span(marker.span, value.span))
        ...
        if self.stream.match(TokenKind.EQUAL) is not None:
            ...
            return LclKeywordArgument(
                ...
                value,
                span=_merge_span(candidate.span, value.span),
            )
...
def parse_primaries(
    ...
    """
    return _PrimaryParser(stream, parse_nested).parse(value)
