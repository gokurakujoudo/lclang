...
from pylcl.lang.lexer.fstring_values import (
    FStringField,
    FStringMatch,
    FStringText,
    FStringValue,
...
class _FStringScanner:
    ...
    def scan(self) -> FStringMatch:
        ...
        .. note::
           ...
           closing delimiter.
        """
        parts = self._parts(top_level=True)
        return FStringMatch(FStringValue(tuple(parts), self.raw), self.cursor)

    def _parts(self, *, top_level: bool) -> list[FStringText | FStringField]:
    ...
    def _field(self) -> FStringField:
        ...
        while self.cursor < len(self.text):
            character = self.text[self.cursor]
            if character in "'\"":
                self.cursor = self._skip_quoted(self.cursor)
                continue
            if character in "([{":
            ...
            if not stack and character in "!:}" and not is_not_equal:
                break
            if not stack and character == "=" and self._is_debug_equal(start):
                debug = True
                break
            if character == "\\":
        ...
        if self._peek() == ":":
            self.cursor += 1
            format_spec = FStringValue(tuple(self._parts(top_level=False)), self.raw)
        elif self._peek() == "}":
            self.cursor += 1
        else:
    ...
    def _skip_quoted(self, start: int) -> int:
...
def scan_fstring(text: str, quote_at: int, *, raw: bool) -> FStringMatch:
    ...
    """
    return _FStringScanner(text, quote_at, raw=raw).scan()
