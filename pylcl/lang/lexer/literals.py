...
from pylcl.lang.lexer.fstrings import FStringScanError, scan_fstring
from pylcl.lang.lexer.tokens import TokenKind
...
class _LiteralScanError(Exception):
...
def scan_literal(text: str, start: int) -> LiteralMatch | None:
    ...
    :raises _LiteralScanError: If a candidate literal is malformed.
    ...
    if character.isdigit() or (character == "." and _peek(text, start + 1).isdigit()):
        return _scan_number(text, start)
    prefix, quote_at = _string_prefix(text, start)
    if quote_at is None:
        return None
    if "f" in prefix.lower():
        try:
            ...
            raise _LiteralScanError(error.message, error.end) from error
        return LiteralMatch(
            TokenKind.FSTRING,
    ...
    return _scan_string(text, start, prefix, quote_at)


def _scan_number(text: str, start: int) -> LiteralMatch:
    ...
    :raises _LiteralScanError: If the candidate is not valid numeric syntax.
    ...
    raise _LiteralScanError("invalid numeric literal", candidate_match.end())


def _string_prefix(text: str, start: int) -> tuple[str, int | None]:
...
def _scan_string(text: str, start: int, prefix: str, quote_at: int) -> LiteralMatch:
    ...
    :raises _LiteralScanError: If the literal is unterminated or malformed.
    ...
    while cursor < len(text):
        if text.startswith(delimiter, cursor):
            ...
            try:
                ...
                raise _LiteralScanError(error.message, content_start + error.end) from error
            kind = TokenKind.BYTES if "b" in prefix.lower() else TokenKind.STRING
            return LiteralMatch(kind, text[start:end], value, end)
        if text[cursor] in "\r\n" and not triple:
            raise _LiteralScanError("newline in single-quoted literal", cursor + 1)
        if text[cursor] == "\\":
            cursor += 1
            if cursor < len(text) and text[cursor] == "\r" and _peek(text, cursor + 1) == "\n":
                cursor += 1
        cursor += 1
    raise _LiteralScanError("unterminated string literal", len(text))
