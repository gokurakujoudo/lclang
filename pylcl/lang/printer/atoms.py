...
def _joined(node: LclJoinedString, render: Render) -> str:
    ...
    for value in node.values:
        if isinstance(value, LclStringText):
            parts.append(_ftext(value.text))
            ...
            if value.format_spec is not None:
                field += f":{_joined(value.format_spec, render)}"
