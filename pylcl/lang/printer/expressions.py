...
from pylcl.ast.operators import BinaryOperator, BooleanOperator, UnaryOperator
...
def render_expression(node: LclAstNode, render: Render) -> RenderResult | None:
    ...
    if isinstance(node, LclUnary):
        return _unary(node, render)
    if isinstance(node, LclBinary):
        return _binary(node, render)
...
def _unary(node: LclUnary, render: Render) -> RenderResult:
...
def _binary(node: LclBinary, render: Render) -> RenderResult:
