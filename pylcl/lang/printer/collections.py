...
from pylcl.ast.comprehensions import LclComprehensionClause
...
def render_collection(node: LclAstNode, render: Render) -> RenderResult | None:
    ...
    if isinstance(node, LclTuple):
        return _tuple(node, render), COLLECTION_PRECEDENCE
    if isinstance(node, LclList):
        return f"[{_sequence(node.elements, render)}]", COLLECTION_PRECEDENCE
    if isinstance(node, LclSet):
        return f"{{{_sequence(node.elements, render)}}}", COLLECTION_PRECEDENCE
    if isinstance(node, LclDict):
        entries = ", ".join(_entry(value, render) for value in node.entries)
        return f"{{{entries}}}", COLLECTION_PRECEDENCE
    if isinstance(node, LclGenerator):
        text = f"({_head(node.element, render)}{_clauses(node.clauses, render)})"
    ...
    if isinstance(node, LclDictComprehension):
        text = f"{{{_entry(node.entry, render)}{_clauses(node.clauses, render)}}}"
        return text, COLLECTION_PRECEDENCE
    return None


def _tuple(node: LclTuple, render: Render) -> str:
    ...
    if not node.elements:
        return "()"
    values = ", ".join(_head(element, render) for element in node.elements)
    suffix = "," if len(node.elements) == 1 else ""
    return f"({values}{suffix})"


def _sequence(values: tuple[LclAstNode, ...], render: Render) -> str:
    ...
    """
    return ", ".join(_head(value, render) for value in values)


def _head(node: LclAstNode, render: Render) -> str:
...
def _entry(node: LclAstNode, render: Render) -> str:
...
def _clauses(clauses: tuple[LclComprehensionClause, ...], render: Render) -> str:
