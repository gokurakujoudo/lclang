...
def _index(node: LclAstNode, render: Render) -> str:
    ...
    if isinstance(node, LclSlice):
        return _slice(node, render)
    return render(node, 0)


def _slice(node: LclSlice, render: Render) -> str:
