...
def render_form(node: LclAstNode, render: Render) -> RenderResult | None:
    ...
    if isinstance(node, LclFunction):
        parameters = ", ".join(internal_parameter(value, render) for value in node.parameters)
        return f"({parameters}) -> {render(node.body, 0)}", FORM_PRECEDENCE
    ...
    if isinstance(node, LclTry):
        return internal_try(node, render), FORM_PRECEDENCE
    if isinstance(node, LclWith):
        items = ", ".join(internal_with_item(value, render) for value in node.items)
        return f"with {items}: {render(node.body, 0)}", FORM_PRECEDENCE
    return None


def internal_parameter(node: LclParameter, render: Render) -> str:
...
def internal_try(node: LclTry, render: Render) -> str:
    ...
    text += "".join(internal_handler(handler, render) for handler in node.handlers)
    if node.finally_body is not None:
        text += f" finally: {render(node.finally_body, 0)}"
    return text


def internal_handler(node: LclExceptHandler, render: Render) -> str:
...
def internal_with_item(node: LclWithItem, render: Render) -> str:
