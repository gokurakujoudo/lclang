...
def decode_content(content: str, *, raw: bool, bytes_mode: bool) -> str | bytes:
    ...
    while cursor < len(content):
        ...
        if character != "\\":
            ...
            cursor += 1
            continue
        value, cursor = internal_decode_escape(content, cursor + 1, bytes_mode=bytes_mode)
        decoded.append(value)
    joined = "".join(decoded)
    return internal_encode_bytes(joined) if bytes_mode else joined


def internal_decode_escape(content: str, cursor: int, *, bytes_mode: bool) -> tuple[str, int]:
