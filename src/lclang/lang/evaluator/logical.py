...
from lclang.ast.operators import BooleanOperator
...
async def internal_evaluate_logical(
    node: LogicalNode,
    resolver: Resolver,
    ...
    if isinstance(node, LclBoolean):
        return await internal_boolean(node, resolver, evaluate)
