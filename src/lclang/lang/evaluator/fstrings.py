...
from lclang.ast import LclFormattedValue, LclJoinedString, LclStringText
...
async def internal_evaluate_joined(
    ...
    for part in node.values:
        if isinstance(part, LclStringText):
            ...
            parts.append(
                await internal_format_value(cast(LclFormattedValue, part), resolver, evaluate)
            )
    return "".join(parts)


async def internal_format_value(
