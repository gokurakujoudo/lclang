...
class InternalControlFormParser:
    ...
    def parse(self) -> LclAstNode | None:
        ...
        if self.stream.current.kind is TokenKind.KW_TRY:
            return self.internal_try()
        if self.stream.current.kind is TokenKind.KW_WITH:
            return self.internal_with()
        return None

    def internal_try(self) -> LclTry:
        """Parse a try body, its handlers, and an optional finally body.

        :returns: Complete :class:`~lclang.ast.LclTry` node with source span.
    ...
    def internal_with(self) -> LclWith:
        ...
        :returns: Complete :class:`~lclang.ast.LclWith` node with source span.
...
def parse_control_form(
    ...
    """
    return InternalControlFormParser(
        stream,
        parse_complete,
