...
class InternalLogicalParser:
    ...
    def parse(self) -> LclAstNode:
        ...
        """
        return self.internal_conditional() if self.allow_conditional else self.internal_coalesce()

    def internal_conditional(self) -> LclAstNode:
    ...
    def internal_or(self) -> LclAstNode:
        ...
        """
        return self.internal_boolean(TokenKind.KW_OR, BooleanOperator.OR, self.internal_and)

    def internal_and(self) -> LclAstNode:
        ...
        """
        return self.internal_boolean(TokenKind.KW_AND, BooleanOperator.AND, self.internal_not)

    def internal_boolean(
        self,
        kind: TokenKind,
    ...
    def internal_not(self) -> LclAstNode:
        ...
        if marker is None:
            return self.internal_comparison()
        operand = self.internal_not()
        return LclUnary(
            UnaryOperator.NOT,
            operand,
    ...
    def internal_comparison(self) -> LclAstNode:
        ...
        while (operator := self.internal_comparison_operator()) is not None:
