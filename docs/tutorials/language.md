...
but lclang uses its own lexer, AST, parser, and async interpreter—never Python
...
see the [complete V1 syntax](../reference/language.md).
...
assert lclang.evaluate_sync("2 + 3 * 4") == 14
...
async def main() -> None:
    expression = lclang.parse_expression("double(value) + 1")
    assert await lclang.evaluate(expression, {"double": double, "value": 21}) == 43
...
Functions are values and use these arrow forms:

| Source | Meaning |
|---|---|
| `() -> expression` | no parameters |
| `name -> expression` | one required positional parameter shorthand |
| `(name) -> expression` | one required positional parameter |
| `(parameters) -> expression` | complete parameter signature |

Canonical source always includes signature parentheses, so `name -> name`
prints as `(name) -> name`. Full signatures support positional parameters,
creation-time defaults, variadic positional parameters, keyword-only parameters
after `*args`, and variadic keyword parameters.

```lcl
(value, scale=2, *extras, clamp=true, **options) -> value * scale
```
...
Calls are async even when every operation inside is synchronous. Arrow functions
are complete-expression forms: nested functions associate through the body, as
shown above, and applying a function expression requires grouping:

```lcl
((value) -> value * 2)(21)
```

The arrow is one contiguous token; whitespace around it is optional.
...
`(a) -> f"{a}-{lhs()}"` and `y` defined as `x(lhs())`, evaluation returns
...
```lcl
recursive((again) -> (n) -> 1 if n <= 1 else n * again(n - 1))
```
...
An LCL builder appears as `Recursive Function: (again) -> ...`; when Python
...
builder. The small `(value) -> ...` wrapper is the eta expansion that delays
the next self-application until the recursive call is actually needed.

```lcl
(f) -> ((x) -> f((value) -> x(x)(value)))((x) -> f((value) -> x(x)(value)))
```
...
```lcl
Y((again) -> (n) -> 1 if n <= 1 else n * again(n - 1))
```

The same Y combinator can derive Fibonacci:

```lcl
Y((again) -> (n) -> n if n <= 1 else again(n - 1) + again(n - 2))
```
...
```lcl
Z((again) -> (items) -> [] if not items else again([item for item in items[1:] if item < items[0]]) + [items[0]] + again([item for item in items[1:] if item >= items[0]]))
```
...
async def main() -> None:
    y = await lclang.evaluate(lclang.parse_expression(Y_SOURCE))
    factorial = await lclang.evaluate(
        lclang.parse_expression(
            "Y((again) -> (n) -> 1 if n <= 1 else n * again(n - 1))"
        ),
        {"Y": y},
    )
    fibonacci = await lclang.evaluate(
        lclang.parse_expression(
            "Y((again) -> (n) -> n if n <= 1 else "
            "again(n - 1) + again(n - 2))"
        ),
        {"Y": y},
    )
    assert await lclang.evaluate(
        lclang.parse_expression("factorial(6)"), {"factorial": factorial}
    ) == 720
    assert await lclang.evaluate(
        lclang.parse_expression("fibonacci(10)"), {"fibonacci": fibonacci}
    ) == 55

    z = await lclang.evaluate(lclang.parse_expression(Z_SOURCE))
    quicksort = await lclang.evaluate(
        lclang.parse_expression(
    ...
    assert await lclang.evaluate(
        lclang.parse_expression("sort(values)"),
...
loops, the Python `lambda` keyword, explicit `await`, `yield`, or Python code
injection. Use expressions, comprehensions, arrow functions, `try`, `with`,
reviewed host values, and `.lclcfg` definitions instead.

Continue with [configuration files](configuration.md) to give these expressions
