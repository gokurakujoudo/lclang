...
A `.lclcfg` file gives names to LCL expressions. lclang parses every file into an
...
Each non-final fragment needs its own marker. lclang removes the markers and
...
async def main() -> None:
    with TemporaryDirectory(prefix="lclang-config-guide-") as directory:
        root = Path(directory)
...
untrusted expressions as if lclang were security isolation.

For every edge case and API contract, continue with the
[detailed `.lclcfg` reference](../reference/configuration.md). To understand the runtime
