...
pylcl can answer two different questions about a definition:
...
async def main() -> None:
    parent = pylcl.Frame(
        pylcl.define_module("rates", {"tax": "subtotal * rate"}),
        "rates",
        values={"rate": 0.2},
    )
    child = pylcl.Frame(
    ...
    try:
        ...
        assert first_tax.definition_path == [
            pylcl.FrameId("invoice"),
            pylcl.FrameId("rates"),
        ]
        assert first_tax.defined_at is parent
        assert first_tax.dependencies[0].definition_path == [pylcl.FrameId("rates")]
...
```python
import pylcl
from pylcl.runtime import DependencyKind, build_dependency_graph, topological_order
...
import pylcl
from pylcl.runtime import FrameDependencyGraph, build_dependency_graph
