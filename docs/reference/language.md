...
LCL V1 is lclang's versioned, expression-only configuration language. It borrows
...
`false`, `none`, `raise`, `try`, `except`, `finally`, `assert`, `with`, and
...
| Forms | `->`, `raise`, `assert`, `try`, `with` | complete-expression forms |
...
task is prohibited; concurrent calls in different tasks are independent. The
bare-name spelling is limited to one required positional parameter. Canonical
printing always emits parenthesized signatures, and applying a function form
requires grouping, as in `((value) -> value)(1)`.
...
<!-- lcl-composite -->
```lcl
(items, *extras, limit=10, **options) -> [*items, *extras][:limit]
```
...
- the Python `lambda` keyword, `async def`, explicit `await`, `yield`, and `yield from`;
