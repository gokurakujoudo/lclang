"""Executable acceptance tests for the workflow tutorial."""
...
def test_workflow_tutorial_examples_execute() -> None:
    ...
    path = ROOT / "docs" / "tutorials" / "workflow.md"
    examples = executable_examples(path.read_text(encoding="utf-8"))
    assert len(examples) == 2
    for source in examples:
        exec(compile(source, "<workflow-tutorial>", "exec"), {})
