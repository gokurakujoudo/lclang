...
async def invoke_parent(
    context: TaskContext, args: ParentArgs, status_mgr: ExecutionStatusManager,
) -> Value:
...
async def run_parent(
    callback: Callable[[TaskContext, ExecutionStatusManager], Awaitable[Value]],
    frame: Frame, children: list[TaskNode] | None = None,
) -> WorkflowExecutionResult:
    """Execute one parent using deterministic inherited metadata."""
    root = define_task("parent", "Parent", task_action=invoke_parent,
                       args_mapping=ParentArgs(callback), children=children or ())
    return await define_workflow("Parent workflow", root).execute(WorkflowExecutionContext(
        True, date(2026, 9, 23), True, logging.getLogger("workflow-call"), frame,
    ))


def make_child(
    status: ExecutionStatus = ExecutionStatus.SUCCESS, *, grouped: bool = False,
) -> Workflow:
    ...
    node = define_task(
        "child", "Child", task_action=execute,
