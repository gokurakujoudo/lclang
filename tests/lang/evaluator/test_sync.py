"""Unit tests mirroring :mod:`pylcl.lang.evaluator.sync`."""
...
def test_evaluate_sync_owns_a_private_loop() -> None:
    ...
    assert pylcl.evaluate_sync(parse_expression("answer"), {"answer": 42}) == 42
...
def test_evaluate_sync_prefers_source_text_and_retains_structured_syntax_errors() -> None:
    ...
    assert pylcl.evaluate_sync("answer + 1", {"answer": 41}) == 42
    with pytest.raises(pylcl.LclSyntaxError):
        pylcl.evaluate_sync("1 +")
...
def test_root_exports_the_sole_sync_boundary() -> None:
    ...
    assert "evaluate_sync" in pylcl.__all__
